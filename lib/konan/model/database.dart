...
class DatabaseHelper {
  ...
  _performDbOperationsVersionWise(Database db, int version) async {
    ...
    case 2:
        await _addStatutLivraisonArtisan(db);
        break;
      ...
      case 5:
        await _addQualificationColumnToEntreprise(db);
        break;
  ...
  Future _addStatutLivraisonArtisan(Database db) async {
    await db.execute('ALTER TABLE artisan ADD COLUMN statut_livraison INTEGER');
    // Init that :
    await db.execute('UPDATE artisan SET statut_livraison = 0');
  }
  ...
  Future _addQualificationColumnToEntreprise(Database db) async {
    await db.execute('ALTER TABLE entreprise ADD COLUMN qualification TEXT');
    await db.execute("UPDATE entreprise SET qualification = ''");
  }
